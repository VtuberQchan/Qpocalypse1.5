...
ISQPTourniquet = ISBaseTimedAction:derive("ISQPTourniquet");

function ISQPTourniquet:isValid()
...
function ISQPTourniquet:waitToStart()
...
function ISQPTourniquet:update()
    if self.character ~= self.otherPlayer then
        self.character:faceThisObject(self.otherPlayer)
    end
...
function ISQPTourniquet:stop()
    ISHealthPanel.setBodyPartActionForPlayer(self.otherPlayer, self.bodyPart, nil, nil, nil)
    ISBaseTimedAction.stop(self);
    self.item:setJobDelta(0.0);
end

function ISQPTourniquet:perform()
    self.item:setJobDelta(0.0);
...
function ISQPTourniquet:new(doctor, otherPlayer, item, bodyPart)
    local o = {}
    setmetatable(o, self)
    self.__index = self
    o.character = doctor;
